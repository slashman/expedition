	...
	public void wearOutShips(int chance, boolean willingly) {
		if (getMovementMode() == MovementMode.SHIP){
			...
			for (Vehicle vehicle: vehiclesToRemove){
				OverworldExpeditionCell cell = (OverworldExpeditionCell) getLevel().getMapCell(getPosition());
				if (cell.isRiver()){
					UserInterface.getUI().showImportantMessage(getEventDatePreffix()+"We lost "+vehicle.getName()+".");
				} else {
					UserInterface.getUI().showImportantMessage(getEventDatePreffix()+"We lost "+vehicle.getName()+" to the sea.");
				}
        ...
        if (absCell instanceof ExpeditionCell){
	        ...
	        if (cell.getStepCommand() != null){
	        	if (cell.getStepCommand().equals("DEPARTURE")){
	        		...
	        		((ExpeditionUserInterface)UserInterface.getUI()).showBlockingMessage("A charriot takes you to the Alcazar of C�rdova");
	        	}
	        }
        } else {
